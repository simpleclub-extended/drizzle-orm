...
describe('spanner to camel case', () => {
    beforeEach(() => {
    ...
    it('insert', ({expect}) => {
        ...
        expect(query.toSQL()).toEqual({
            sql: 'insert into `users` (`id`, `firstName`, `lastName`, `AGE`) values (default, @p1, @p2, @p3)',
            params: ['John', 'Doe', 30],
        });
        expect(db.dialect.casing.cache).toEqual(usersCache);
    });
    ...
    it('insert (on duplicate key update)', ({expect}) => {
        ...
        expect(query.toSQL()).toEqual({
            sql:
                'insert or update into `users` (`id`, `firstName`, `lastName`, `AGE`) values (default, @p1, @p2, @p3)',
            params: ['John', 'Doe', 30],
        });
        expect(db.dialect.casing.cache).toEqual(usersCache);
    });
    ...
    it('update', ({expect}) => {
        ...
        expect(query.toSQL()).toEqual({
            sql: 'update `users` set `firstName` = @p1, `lastName` = @p2, `AGE` = @p3 where `users`.`id` = @p4',
